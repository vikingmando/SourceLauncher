...
module.exports.getManifest = function(mods, fullScan, nexusPath, checkFiles) {
    if (!mods) mods = [];
    var files = require('./required');
    if (fullScan || nexusPath && !fs.existsSync(path.join(nexusPath, "client.cfg"))) {
        //force download with size:0, md5:""
        files = files.concat([{
                name: "client.cfg",
                size: 0,
                md5: "",
                url: "http://swgnexus.ddns.net/swg/launcher/client.cfg"
            },
            {
                name: "local_machine_options.iff",
                size: 0,
                md5: "",
                url: "http://swgnexus.ddns.net/swg/launcher/local_machine_options.iff"
            },
            {
                name: "preload.cfg",
                size: 0,
                md5: "",
                url: "http://swgnexus.ddns.net/swg/launcher/preload.cfg"
            }
        ]);
    }
    request({
...
var forks = [];
var canceling = true;
module.exports.install = function(swgPath, nexusPath, mods, fullScan) {
    console.log('installer');
    const child_process = require('child_process');
    canceling = false;
    module.exports.getManifest(mods, fullScan, nexusPath, checkFiles);
...
if (process.send) {
    ...
    mkdirp(process.env.nexusPath);

    process.on('message', (fileInfo) => {
        let src = path.join(process.env.swgPath, fileInfo.name);
        let dst = path.join(process.env.nexusPath, fileInfo.name);
        ...
        fs.stat(src, (err, stats) => {
            ...
            if (stats.size != fileInfo.size) {
                process.send("size mismatch actual: " + stats.size + ' expected: ' + fileInfo.size);
                return doDownload();
            }
        ...
        function complete(err) {
            ...
            process.send({
                'complete': expectedSize - progressReported
            });
        }
    ...
    function unzip(dest, complete) {
        ...
        zip.extractAllToAsync(process.env.nexusPath, true, complete);
    }
