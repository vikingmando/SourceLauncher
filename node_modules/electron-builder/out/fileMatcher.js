...
function createFileMatcher(appDir, resourcesPath, macroExpander, platformSpecificBuildOptions, packager, outDir) {
    const buildResourceDir = _path.resolve(packager.info.projectDir, packager.buildResourcesDir);
    const matchers = packager.info.isPrepackedAppAsar ? null : getFileMatchers(packager.info.config, "files", appDir, _path.join(resourcesPath, "app"), false, macroExpander, platformSpecificBuildOptions);
    const matcher = matchers == null ? new FileMatcher(appDir, _path.join(resourcesPath, "app"), macroExpander) : matchers[0];
    ...
    if ((0, (_fileTransformer || _load_fileTransformer()).hasDep)("lzma-native", packager.info)) {
        patterns.push("!**/node_modules/lzma-native/build{,/**/*}");
        patterns.push("!**/node_modules/lzma-native/deps{,/**/*}");
    }
    if ((0, (_fileTransformer || _load_fileTransformer()).hasDep)("keytar-prebuild", packager.info)) {
        patterns.push("!**/node_modules/keytar-prebuild/src{,/**/*}");
    }
    if ((0, (_fileTransformer || _load_fileTransformer()).hasDep)("keytar", packager.info)) {
    ...
    patterns.push("!**/{.git,.hg,.svn,CVS,RCS,SCCS," + "__pycache__,.DS_Store,thumbs.db,.gitignore,.gitkeep,.gitattributes," + ".idea,.vs,.editorconfig,.flowconfig,.jshintrc,.eslintrc," + ".yarn-integrity,.yarn-metadata.json,yarn-error.log,yarn.lock,package-lock.json,npm-debug.log," + "appveyor.yml,.travis.yml,circle.yml,.nyc_output}");
    return matcher;
}
/** @internal */
function getFileMatchers(config, name, defaultSrc, defaultDestination, allowAdvancedMatching, macroExpander, customBuildOptions) {
